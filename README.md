...
The playground for this workshop was a set of projects designed to help English-speaking students learn Japanese. Five weeks of projects integrating generative AI tools included a language learning portal, a podcast transcription and translation app, a Kanji learning game, a containerized text generation service, and a visual novel.
...
Key Notes for this work:
  ...
  - By forcing this constraint, this project served as a training exercise in prompt engineering and learning how to use AI tools to generate code and other artifacts. This made the workshop essentially a crash course in **"vibe coding"***... a term that, funny enough, only [emerged in the public consciousness](https://x.com/karpathy/status/1886192184808149383) concurrently with the workshop.
...
- Backend API Implementation
    - I wrote the backend from scratch in FastAPI, which I have never used before and took it as an extra challenge. This included scripts for constructing and seeding the database, with management via Alembic (also new to me). I do work with Python, but mostly in Jupyter notebooks.
...
- Frontend API Implementation
    - I wrote the frontend from scratch in React and TailwindCSS, which we manage in development via a Vite server.
...
- Language Listening App
    ...
    - Files can be processed for both time-coded transcription and time-coded English translation using OpenAI API.
    ...
    - App is written using Streamlit and TinyDB, written entirely with no manual coding, all AI-developed.
- Kana Practice App
    ...
    - Kanji Snake was the demonstration of this process, written in Phaser and starting with a template available from the Lang Portal that provides libraries for the game engine to interact with the Lang Portal's backend.
...
- Describe your progress with OPEA
    ...
    - As with everything up to date, I have written zero code myself - everything is AI created.
...
- Technical Achievements
    - Fully AI-Generated Development - The entire project (code, artwork, game design, dialogue) was created using AI with no direct human programming or asset creation
    - Robust Character System - Implemented a flexible character management system with:
        - Character positioning (left, center, right, offscreen)
        - Multiple emotion states per character
        - Character sprite loading and management
    - Advanced Dialog System - Created a sophisticated dialog system featuring:
        - Japanese text with proper line breaking rules
        - Support for romaji transliteration and English translations
        - Context-sensitive difficulty levels (beginner, intermediate, advanced)
    - Hot Module Replacement Integration - Implemented state-preserving HMR for Phaser allowing:
        - Game state persistence across hot reloads
        - Scene-specific reloading without losing progress
        - Seamless developer experience with instant updates
    - LLM Proxy Server - Built a secure, provider-agnostic LLM proxy that:
        - Protects API keys and sensitive information
        - Supports multiple providers (OpenAI, Anthropic, local models)
        - Handles intelligent JSON response extraction (from models that are a little too chatty)
        - Can be configured via environment variables
    - Dynamic Study System - Implemented an LLM-powered study system that:
        - Provides detailed grammar breakdowns
        - Offers alternative expressions and usage contexts
        - Explains cultural nuances
        - Gives pronunciation tips and highlights common mistakes
    - Japanese Text Processing - Created specialized utilities for:
        - Proper Japanese text wrapping following linguistic rules that respects Japanese typographic conventions
    - Visual Novel Gameplay Mechanics - Implemented core VN mechanics including:
        - Background scene transitions between locations
        - Adaptive dialog UI with different sized response boxes based on text length
        - Progress tracking across game sessions
    - Test Scene Integration - Created a comprehensive test environment that:
        - Allows testing of character positioning and emotions
        - Provides debugging tools for dialog system
        - Supports rapid iteration and verification of features
    - Multi-Location Narrative Flow - Structured a complete narrative experience across:
        - Multiple locations (train station, restaurant, park, mall, hotel)
        - Character interactions with multiple NPCs
        - Progressive learning objectives tied to narrative contexts
- Learning Aspects
    - Progressive Language Acquisition - The game implements a natural learning progression where:
        - Japanese phrases are introduced in realistic contexts
        - Cultural nuances are explained alongside language
        - Multiple difficulty levels support learners at different stages
    - Context-Based Learning - Language learning occurs within realistic scenarios:
        - Ordering food at restaurants
        - Navigating train stations
        - Shopping conversations
        - Social interactions in various settings
    - AI-Powered Personalized Learning - The LLM integration provides:
        - Dynamic explanations tailored to player interests
        - Detailed grammar breakdowns based on inquiry
        - Cultural context that enhances language understanding
        - Alternative expressions for different situations
    - Difficulty Scaling - The three-tiered difficulty system:
        - Gradually removes training wheels as players progress
        - Adjusts UI to match player capability
