...
- Edit the `docs/Prompt-Header.md` file to apply to the new project (conda env, etc.)
- Move any applicable files from the `/dev/template/docs/reference` directory to the `/docs/reference` directory
- Delete the `dev/template` folder (optional)
- Run `scripts/update_docs.py` script again

- Create a template header for new chats below
