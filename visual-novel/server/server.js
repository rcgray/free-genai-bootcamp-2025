 ...
 * @returns {Object} The adapted request body
 */
 ...
 * @returns {Object} The processed response with valid JSON in content
 */
function processLLMResponse(responseData) {
  ...
  console.log('[LLM-PROXY] Processing LLM response content');
  ...
  try {
    ...
    try {
      ...
      return responseData; // Already valid JSON, no processing needed
    } catch (e) {
    ...
    processedResponseData.choices[0].message.processed_by_proxy = true;
    
    console.log('[LLM-PROXY] Successfully processed and extracted valid JSON');
    return processedResponseData;
  } catch (error) {
    console.error('[LLM-PROXY] Failed to extract valid JSON from response:', error.message);
    
    // If we failed to extract valid JSON, log the error but return the original content
    return responseData;
  }
}
...
app.post('/api/completions', async (req, res) => {
  try {
    ...
    requestBody = adaptRequestForProvider(requestBody);
    ...
    console.log(`[LLM-PROXY] Using model: ${requestBody.model}`);
    
    // Optionally log the full request for debugging
    if (process.env.DEBUG_MODE === 'true') {
      console.log('[LLM-PROXY] Request body:', JSON.stringify(requestBody, null, 2));
    }
    ...
    // Process the response to extract valid JSON if needed
    const processedResponseData = processLLMResponse(response.data);
