...
async def test_create_word(client: AsyncClient, db: AsyncSession):
    response = await client.post("/api/v1/words", json=TEST_WORD)
...
async def test_create_duplicate_word(client: AsyncClient, db: AsyncSession):
    # Create first word
    await client.post("/api/v1/words", json=TEST_WORD)
...
async def test_get_words_pagination(client: AsyncClient, db: AsyncSession):
    ...
    await client.post("/api/v1/words", json=TEST_WORD)
    await client.post("/api/v1/words", json=TEST_WORD_2)

    # Test pagination
    response = await client.get("/api/v1/words?page=1&per_page=1")
    assert response.status_code == 200
...
async def test_get_word(client: AsyncClient, db: AsyncSession):
    # Create word
    create_response = await client.post("/api/v1/words", json=TEST_WORD)
    word_id = create_response.json()["data"]["id"]

    # Get word
    response = await client.get(f"/api/v1/words/{word_id}")
    assert response.status_code == 200
    data = response.json()["data"]
...
async def test_get_words_sorting(client: AsyncClient, db: AsyncSession):
    ...
    await client.post("/api/v1/words", json={
        "kanji": "猫",
        "romaji": "neko",
        "english": "cat",
        "parts": [
    ...
    response = await client.get("/api/v1/words?sort_by=romaji&order=asc")
    assert response.status_code == 200
    data = response.json()["data"]
    items = data["items"]
    assert len(items) == 2
    ...
    response = await client.get("/api/v1/words?sort_by=romaji&order=desc")
    assert response.status_code == 200
    data = response.json()["data"]
    items = data["items"]
    assert len(items) == 2
